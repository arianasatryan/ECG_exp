...
def predict():
    # load data generator
    if data_source != 'both':
        _, _, test_df = data_split_func[data_source](classification_type)
    else:
        _, _, test_df_tis = data_split_func['tis'](classification_type)
        _, _, test_df_ptb = data_split_func['ptb'](classification_type)
        test_df_tis = shuffle(test_df_tis, n_samples=len(test_df_ptb), random_state=SEED)
